...
def mptt_items_for_result(cl,result,form):
	...
	for field_name in cl.list_display:
		...
		if (first and not cl.list_display_links) or field_name in cl.list_display_links:
			...
			yield mark_safe('<%s%s%s><a href="%s"%s>%s</a></%s>'%(table_tag,row_class,padding_attr,url,(
		...
		##### MPTT SUBSTITUTION END
			...
			if (form and field_name in form.fields and not (
					field_name == cl.model._meta.pk.name and form[cl.model._meta.pk.name].is_hidden)):
				bf = form[field_name]
...
def mptt_result_list(cl):
	...
	"""
	return {'cl':cl,'result_hidden_fields':list(result_hidden_fields(cl)),'result_headers':list(result_headers(cl)),
